...
export type FetchResult<T> =
  ...
  | { ok: false; status: number; resetUnix?: number | null; error?: string }
...
async function ghFetch<T>(path: string): Promise<FetchResult<T>> {
  ...
  if (res.status === 403 || res.status === 429) {
    return { ok: false, status: res.status, resetUnix: readReset(res) }
  }
  if (!res.ok) {
    return { ok: false, status: res.status, error: `github ${res.status}` }
  }
...
export async function fetchAllRepos(
  ...
  while (true) {
    const q = new URLSearchParams({ type: "public", per_page: "100", page: String(page) })
    const r = await ghFetch<GitHubRepo[]>(`/orgs/${encodeURIComponent(slug)}/repos?${q}`)
    if (!r.ok) return r
...
export async function fetchTopContributors(
  ...
  for (const repo of topRepos) {
    ...
    if (!r.ok) {
      if (r.status === 404) { lists.push([]); continue }
      return r
    }
    lastRemaining = r.rateRemaining ?? lastRemaining
    lists.push(r.data)
